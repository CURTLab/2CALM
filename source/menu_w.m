function k = menu_w(xHeader,varargin)
...
textPadding = [28 15];   % extra [Width Height] on uicontrols to pad text
uiGap       = 3;       % space between uicontrols
uiBorder    = 8;       % space between edge of figure and any uicontrol
winTopGap   = 30;       % gap between top of screen and top of figure **
winLeftGap  = 1;       % gap between side of screen and side of figure **
winWideMin  = 240;      % minimum window width necessary to show title %%%%%%%%%%%wj
...
%------------------------------------------------------------------------
menuFig = figure( 'WindowStyle', 'normal', ...  %%%'modal'
                  'Units'       ,MenuUnits, ...
                  'Visible'     ,'on', ...
                  'NumberTitle' ,'off', ...
                  'Name'        ,'2CALM-SERIE',...           %%%getString(message('MATLAB:uistring:menu:MENU')), ...
                  'Resize'      ,'off', ...
                  'Colormap'    ,[], ...
                  'MenuBar'     ,'none',...
                  'ToolBar' 	,'none' ...
...
textWide  = 1.0*maxsize(3);
textHigh  = 1.1*maxsize(4);
%------------------------------------------------------------------------
...
for idx = numItems : -1 : 1; % start from top of screen and go down
    ...
    hBtn(n) = uicontrol( ...
               ...
               'String'         , xcItems{n},...
               'BackgroundColor' ,[0.7 0.7 0.4]);%get(menuFig,'color'));
...
winWide = 1*numCols*(btnWide) + (numCols - 1)*uiGap + 2*uiBorder; %%%%%%%
...
bottom = screensize(4) - (winHigh + winTopGap)-0.0;

% Set figure window position
set( menuFig, 'Position', [winLeftGap+10 bottom winWide winHigh] );%%%%%%%%%%%%%%%%%%%%%%%wj
...
uiPosMtx = [ xPos(:), yPos(:), 1.1*btnWide*allBtn, btnHigh*allBtn ];%%%
cUIPos   = num2cell( uiPosMtx( 1:numItems, : ), 2 );

% adjust all buttons
set( hBtn, {'Position'}, cUIPos, 'HorizontalAlignment','right','FontSize',10,'FontWeight','normal');
